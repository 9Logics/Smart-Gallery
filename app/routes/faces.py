        ...
        try:
            ...
            for idx, path in enumerate(photos):
                ...
                if os.path.exists(path):
                    try:
                        faces = processor.detect_and_extract_faces(path,
                            min_confidence=0.5)
                        for face in faces:
                            bbox = face['bbox']
            ...
            print(f'Error in face rescan: {e}')
        finally:
            if conn is not None:
                try:
                    conn.close()
...
def manual_safe_rescan():
    ...
    def safe_rescan_task():
        ...
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            # NOT EXISTS rather than NOT IN: `path NOT IN (SELECT photo_path ...)`
            # evaluates to NULL (i.e. filters everything out) as soon as a single
            # faces.photo_path row is NULL, silently making this scan a no-op.
            cursor.execute(
                """
                SELECT path FROM photos
                WHERE trashed_at IS NULL
                  AND NOT EXISTS (SELECT 1 FROM faces WHERE faces.photo_path = photos.path)
                """
                )
...
def get_training_pairs():
    ...
    try:
        cursor = conn.cursor()
        # `embedding IS NOT NULL` is required: manually tagged faces are stored with
        # a NULL embedding, and np.frombuffer(None) raises TypeError -> HTTP 500.
        cursor.execute(
            """
        ...
        for nf in named_faces[:10]:
            ...
            for uf in unassigned_faces:
                ...
                if u_emb.shape != n_emb.shape:
                    continue
                # Use the same cosine distance metric the clusterer uses. The old
                # code used raw L2 norm, which is on a different scale entirely, so
                # the 0.4-0.75 "uncertain" window selected essentially random pairs.
                dist = face_processor.compute_cosine_distance(n_emb, u_emb)
                if 0.4 < dist < 0.75:

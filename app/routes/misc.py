...
def resolve_duplicates():
    ...
    try:
        for res in resolutions:
            ...
            if del_path and os.path.exists(del_path):
                ...
                if not success:
                    # The return value was previously discarded, so the row was
                    # marked trashed even when the file never moved - the photo
                    # then vanished from the UI while still occupying disk.
                    trash_errors.append(f'{del_path}: {err}')
                    continue
                cursor.execute(
    ...
    return jsonify({'success': True, 'deleted': len(deleted_paths),
        'renamed': len(renamed_paths), 'errors': trash_errors})
...
def api_memories_curated():
    ...
    # Single quotes, not double: SQLite treats "Me" as an *identifier* first and only
    # falls back to a string literal when no such column exists. That fallback is a
    # legacy compatibility quirk, and it silently breaks the moment `people` gains a
    # column named Me. Use a bound parameter instead.
    c.execute('SELECT id FROM people WHERE name = ? COLLATE NOCASE', ('Me',))
    me_row = c.fetchone()
    if me_row:
        me_id = me_row[0]
    else:

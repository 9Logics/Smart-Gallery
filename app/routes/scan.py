...
def get_scan_status():
    ...
    # deepcopy walks the dict in Python, so it must hold scan_lock: background
    # threads mutate scan_status constantly and an unlocked walk can both tear the
    # snapshot and raise "dictionary changed size during iteration".
    with scan_lock:
...
def cancel_scan():
    from app.app_core import scan_status, scan_lock, scan_queue, scan_queue_lock
    with scan_queue_lock:
        if len(scan_queue) > 1:
            del scan_queue[1:]
    with scan_lock:
...
@scan_bp.route('/api/scan/object-ai', methods=['POST'])
def scan_object_ai():

    def object_scan_task():
        global scan_status
        with scan_lock:
            scan_status['status'] = 'scanning'
            scan_status['cancel_requested'] = False
            scan_status['phase'] = 'Object AI Scan'
            scan_status['processed'] = 0
            scan_status['total'] = 1
            scan_status['current_file'] = ''
        conn = None
    ...
    from app.app_core import queue_scan
    added = queue_scan('object_ai', object_scan_task)
    if not added:
        return jsonify({'success': True, 'message': 'Object AI scan already queued'})
    return jsonify({'success': True, 'message': 'Object AI scan started'})

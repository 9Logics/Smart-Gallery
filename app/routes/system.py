...
def save_scan_folder():
    ...
    conn.commit()
    conn.close()
    # photos.is_path_authorized() memoises the resolved scan root for 30s; drop it
    # now so the new folder takes effect immediately instead of 403-ing thumbnails.
    try:
        from app.routes.photos import invalidate_scan_root_cache
        invalidate_scan_root_cache()
    except Exception as e:
        print(f'Could not invalidate scan-root cache: {e}')
...
def export_cache():
    ...
    os.close(temp_db_fd)
    # shutil.copy2 of a live WAL database copies only the main file and misses
    # everything still sitting in the -wal, producing a stale or torn backup.
    # sqlite3's backup API takes a consistent snapshot instead.
            ...
            if imp_ai:
                ...
                if os.path.exists(hero_src):
                    shutil.copy2(hero_src, os.path.join(CACHE_DIR,
                        'hero_overrides.json'))
            try:
                # Was `import scene_classifier`, which cannot resolve now that the
                # module lives in the `app` package. The ImportError was swallowed
...
def delete_all_data():
    try:
        ...
        try:
            # `import scene_classifier` failed silently here (wrong module path),

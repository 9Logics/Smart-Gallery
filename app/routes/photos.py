        ...
        if search_query:
            ...
            for term in search_terms:
                ...
                where_clauses.append(
                    """
                    (p.filename LIKE ? OR p.place_name LIKE ? OR g.place_name LIKE ? OR p.date_taken LIKE ? OR p.ai_tags LIKE ? OR p.path IN (
                        SELECT f.photo_path FROM faces f JOIN people pe ON f.person_id = pe.id WHERE pe.name LIKE ?
                    ) OR fuzzy_match(?, p.filename) = 1 OR fuzzy_match(?, p.place_name) = 1 OR fuzzy_match(?, p.ai_tags) = 1 OR p.path IN (
...
_scan_root_cache = {'root': None, 'at': 0.0}
_scan_root_lock = threading.Lock()


def _get_scan_root(max_age=30.0):
    """Resolved scan-folder root, memoised for `max_age` seconds.

    is_path_authorized() runs on the thumbnail route, which the virtualised grid
    hits hundreds of times per screenful. Opening a SQLite connection per call
    (and calling realpath, which stats every path component) made authorisation
    the most expensive part of serving a thumbnail.
    """
    now = time.time()
    with _scan_root_lock:
        if _scan_root_cache['root'] is not None and now - _scan_root_cache['at'
            ] < max_age:
            return _scan_root_cache['root']
    conn = get_db_connection()
    try:
        cursor = conn.cursor()
    ...
    if not row or not row[0]:
        return None
    try:
        root = os.path.normcase(os.path.realpath(row[0]))
    except OSError:
        return None
    with _scan_root_lock:
        _scan_root_cache['root'] = root
        _scan_root_cache['at'] = now
    return root


def invalidate_scan_root_cache():
    """Drop the memoised scan root. Call after changing the 'scan_folder' setting."""
    with _scan_root_lock:
        _scan_root_cache['root'] = None
        _scan_root_cache['at'] = 0.0


def is_path_authorized(photo_path):
    """True only if photo_path resolves inside the configured scan folder.

    Uses os.path.commonpath on the real (symlink/junction-resolved) paths rather
    than a str.startswith() prefix test. A prefix test treats "C:\\PhotosLeaked"
    as being inside "C:\\Photos", and does not stop "..' traversal at all.
    """
    root = _get_scan_root()
    if not root:
        return False
    try:
        target = os.path.normcase(os.path.realpath(photo_path))
        if root == target:
            return True
        return os.path.normcase(os.path.commonpath([root, target])) == root
...
def serve_photo_file(photo_path):
    ...
    if not is_path_authorized(photo_path):
...
def serve_photo_thumbnail(photo_path):
    # This route had no authorization check at all, so any absolute path could be
    # read off disk and returned re-encoded as a JPEG.
    photo_path = os.path.normpath(photo_path)
    if not is_path_authorized(photo_path):
        return 'Access denied', 403
...
def api_photo_rename():
    ...
    try:
        ...
        # before BEGIN and restored after the transaction closes.
        ...
        # Put the file back so disk and database cannot disagree.
        if renamed:
            try:
                os.rename(new_path, photo_path)
            except Exception as undo_err:
                print(f'Failed to roll back rename of {new_path}: {undo_err}')
        return jsonify({'error': str(e)}), 500
    finally:
        if conn is not None:
            try:

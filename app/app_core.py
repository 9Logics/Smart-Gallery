...
from app import face_processor
# Setup Flask paths for PyInstaller compatibility
...
# The cache stores whole response bodies. Because the frontend appends a
...
def _api_cache_store(key, body, mimetype):
    if len(body) > API_CACHE_MAX_BODY_BYTES:
        return
    now = time.time()
    with API_CACHE_LOCK:
        API_CACHE[key] = now, body, mimetype
        if len(API_CACHE) > API_CACHE_MAX_ENTRIES:
            # Evict oldest-first until back under the cap. Entries are keyed by
            # insertion time, not by their per-route timeout, so this is a plain LRU
            # -style trim rather than a TTL sweep.
            for stale_key in sorted(API_CACHE, key=lambda k: API_CACHE[k][0])[:
                len(API_CACHE) - API_CACHE_MAX_ENTRIES]:
                API_CACHE.pop(stale_key, None)
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if resp.status_code == 200:
                _api_cache_store(key, resp.get_data(), resp.mimetype)
            return resp
...
def sqlite_tag_match(query, target):
    if not target or not query: return 0
    q = str(query).lower().strip()
    t = str(target).lower()
    
    # Check if target is a JSON array (ai_tags)
    if t.startswith('['):
        try:
            tags = json.loads(t)
            for tag in tags:
                # If the query exactly matches a word in the tag
    ...
    # For regular text (filename, place_name), do word boundary check
...
def get_db_connection():
    ...
    conn.create_function('tag_match', 2, sqlite_tag_match)
    return conn
...
@contextmanager
def db_transaction():
    """Commit-or-rollback AND close a connection.

    `with get_db_connection() as conn:` looks like it closes the connection, but
    sqlite3's own context manager only manages the transaction - the handle stays
    open. Every request that used that idiom leaked a connection (and its WAL read
    lock) until the process exited. Use this instead.
    """
...
def set_scan_idle(phase=''):
    """Return scan_status to a terminal/idle state.

    Every background job MUST call this when it finishes (success, failure or
    cancellation). If it does not, scan_status['status'] stays 'scanning'
    forever, which (a) permanently blocks /api/faces/rescan via its
    "already in progress" guard and (b) pins the frontend status poller at its
    fast 250ms interval for the rest of the process lifetime.
    """
    with scan_queue_lock:
        more_work_queued = len(scan_queue) > 1
    with scan_lock:
        if more_work_queued:
    ...
    def worker():
        ...
        while True:
            ...
            with scan_queue_lock:
                ...
                if scan_queue:
                    scan_queue.pop(0)
                # Safeguard: Keep status as scanning if there's another job in the
                # queue so the frontend poller doesn't prematurely terminate.
                # NOTE: the job has already been popped here, so any remaining
                # element means more work - not `> 1` as in set_scan_idle().
                more_work_queued = bool(scan_queue)
            # scan_status is guarded by scan_lock, not scan_queue_lock. These writes
            # used to happen under the wrong lock, which could tear the deepcopy()
            # snapshot taken by /api/scan/status.
            with scan_lock:
                if more_work_queued:
                    scan_status['status'] = 'scanning'
                    scan_status['phase'] = 'Starting next job...'
...
def reverse_geocode(lat, lon):
    ...
    if row:
        try:
            ...
            return row[0]
    # Nominatim's usage policy allows at most 1 request/second, and exceeding it gets
    # the client IP blocked. `geocode_lock` was declared but never acquired, so the
    # read-modify-write of last_geocode_time raced: N scan threads all read the same
    # timestamp, all saw elapsed >= 1.0, and all fired at once. Hold the lock across
    # the pacing decision, then release it before the (slow) network call so requests
    # are still issued no faster than one per second without serialising entirely.
    with geocode_lock:
    ...
    try:
        ...
        if place_name:
            # db_transaction() commits/rolls back AND closes. The previous inline
            # open/commit/close leaked the handle whenever the INSERT raised, because
            # the failure was swallowed by the broad `except` below.
...
def save_date_to_file_and_system(photo_path, new_date_str):
    ...
    try:
        ...
        # restype/argtypes MUST be declared. Without them ctypes assumes the return
        # value is a 32-bit signed int, so a 64-bit HANDLE gets truncated and then
        # sign-extended - the value handed to SetFileTime/CloseHandle is not the
        # handle Windows returned. It also made the INVALID_HANDLE_VALUE test below
        # unreachable: a c_int result can never equal 18446744073709551615.
        ...
        kernel32.SetFileTime.argtypes = [wintypes.HANDLE, ctypes.POINTER(
            FILETIME), ctypes.POINTER(FILETIME), ctypes.POINTER(FILETIME)]
        kernel32.CloseHandle.restype = wintypes.BOOL
        kernel32.CloseHandle.argtypes = [wintypes.HANDLE]
        INVALID_HANDLE_VALUE = ctypes.c_void_p(-1).value
        handle = kernel32.CreateFileW(photo_path, 1073741824, 1 | 2, None, 3,
            33554432 | 128, None)
        if handle and handle != INVALID_HANDLE_VALUE:
            success = kernel32.SetFileTime(handle, ctypes.byref(ft), ctypes.
                byref(ft), ctypes.byref(ft))
            err = ctypes.get_last_error() if not success else 0
            kernel32.CloseHandle(handle)
            if not success:
                raise Exception(f'SetFileTime failed (GetLastError={err})')
            print(
...
def send_file_to_trash(path):
    """
    Moves a file into the app's internal trash directory (.cache/trash/).
    Uses MD5 hash of the original path as the filename to enable restore.
    Returns: (success_bool, trash_path, error_str_or_None)
    """

    ...
    def detect_and_extract_faces(self, image_path, min_confidence=0.8):
        ...
        if faces is not None:
            ...
            for face in faces:
                ...
                # NMS / IoU Check to prevent duplicate detections of the same face
                boxA = [x, y, x + width, y + height]
                is_duplicate = False
                for boxB in processed_bboxes:
...
def dbscan_clustering(embeddings, eps=0.45, min_samples=2):
    ...
    for i in range(n_samples):
        ...
        visited_in_cluster = set([i]) # Keep track of what we expand
        while len(queue) > 0:
            current = queue.pop(0)
            ...
            if current in visited_in_cluster:
                continue
            visited_in_cluster.add(current)
            
            # Find neighbors of current point
            current_neighbors = np.where(dist_matrix[current] < eps)[0]
            if len(current_neighbors) >= min_samples:
                for n in current_neighbors:
                    if labels[n] == -1 and n not in visited_in_cluster:
                        queue.append(n)

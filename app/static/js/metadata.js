...
function savePhotoDate() {
    ...
    .catch(err => {
        appAlert(err.message || 'Failed to save date');
    })
    .finally(() => {
...
function fixPhotoDateFromFilename() {
    ...
    .then(data => {
        if (data.success) {
            ...
            loadPhotos();
            appAlert("File creation/modification date successfully corrected on disk and database!");
        }
    })
    .catch(err => {
        appAlert("Failed to fix date: " + err.message);
    })
    .finally(() => {
...
function savePhotoLocation() {
    ...
    .catch(err => {
        appAlert(err.message || 'Failed to save location');
    })
    .finally(() => {

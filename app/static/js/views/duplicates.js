...
async function resolveDuplicates() {
    // NOTE: this predicate must stay synchronous. An `async` arrow returns a Promise,
    // which is always truthy, so every group (including unchecked ones) would be
    // resolved and its files permanently deleted.
    ...
    if (!await appConfirm(`Are you sure you want to resolve and clean up duplicates for the ${count} checked duplicate group(s)? This will permanently delete duplicate files from your disk and rename kept copies to remove copy suffixes where necessary.`)) {
        return;
    }
    ...
    .then(data => {
        appAlert(`Successfully resolved duplicates!\nDeleted: ${data.deleted} files.\nRenamed: ${data.renamed} original files to remove copy suffixes.`);
        loadStaticData();
        loadDuplicates();
    })
    .catch(err => {
        appAlert(err.message || 'Failed to resolve duplicates');
...
async function resolveSingleDuplicateGroup(gIndex) {
    const group = state.duplicateGroups[gIndex];
    if (!group) return;
    
    if (!await appConfirm(`Are you sure you want to resolve duplicates for this group? This will permanently delete the suggested file for deletion and rename/keep the selected original.`)) {
        return;
    }
    ...
    .then(data => {
        appAlert("Group resolved successfully!");
        state.duplicateGroups.splice(gIndex, 1);
        loadStaticData();
        renderDuplicates(state.duplicateGroups);
    })
    .catch(err => {
        appAlert(err.message || 'Failed to resolve duplicate group');
    });
}

async function deleteBothDuplicates(gIndex) {
    const group = state.duplicateGroups[gIndex];
    if (!group) return;
    
    if (!await appConfirm(`Are you sure you want to move BOTH files to the Recycle Bin?`)) {
        return;
    }
    ...
    .then(res => res.json())
    .then(data => {
        appAlert("Both copies moved to Recycle Bin successfully.");
        state.duplicateGroups.splice(gIndex, 1);
        loadStaticData();
        renderDuplicates(state.duplicateGroups);
    })
    .catch(err => {
        appAlert("Failed to delete both duplicates: " + err.message);
    });
}
